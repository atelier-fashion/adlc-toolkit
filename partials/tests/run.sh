...
if [ "${1-}" = "--inner" ]; then
  ...
  # under that shell.
  run_all "$2" "$HERE/id-alloc.test.sh" "$HERE/forge.test.sh" "$HERE/attribution.test.sh" "$HERE/intake.test.sh" "$HERE/delegate-gate.test.sh" "$HERE/source-guard.test.sh" "$HERE/conflict-bound.test.sh"
  exit $RC
fi
