...
- [ ] BR-14 (**known limitation, discharged by REQ-604**): the config reader that this REQ makes the sole authority is a hand-written flat reader, and it **fails open on ordinary YAML shapes** — a comment on the `delegate:` header, a BOM, a tab-indented header, a nested mapping containing `enabled`, a second `delegate:` block, a directory at the config path. Each yields `{}`, indistinguishable from no config, and legacy-key continuity then grants against a written `enabled: false`. Every one of these is **pre-existing on `main`**: this REQ does not introduce them and does not fix them. An attempt to fix them in this REQ (commit `bc7d584`, discarded) added six further fail-opens under a tests-first discipline, because a reader that *skips* what it does not understand cannot be patched into one that *refuses*; see `adversary-report.md` passes 3 and 4. A real parser behind a strict schema, with a differential oracle, is REQ-604. **Beside it, the shell resolver**: `command -v` consults shell state, so a shell function (zsh), an alias, or a hash-table entry named `adlc-read` can satisfy binary resolution — also pre-existing on `main`, also REQ-604 (its BR-11). Until REQ-604 lands, the controls that hold are `enabled: false` in a config the minimal reader *can* parse, and the kill switch.
...
- **The config parser, and shell-state-free binary resolution.** Both are REQ-604. This REQ names their present fail-opens as a known limitation in BR-14 rather than claiming them fixed.
