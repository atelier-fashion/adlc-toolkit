...
- [ ] BR-4: the gate's six reason strings and the 0/1/2 return-code shape are preserved, with **four named divergences** — enumerated here because "one deliberate change" was asserted in an earlier draft and was false. Every divergence fails *closed*; none makes the gate grant delegation it previously withheld.
...
- [ ] **Exactly the four BR-4 divergences occur, and no others.** The full (input, reason, rc) matrix is compared against the pre-REQ gate exhaustively rather than spot-checked, with the four named rows marked as expected. A spot-checked comparison is what allowed an unnamed fifth divergence — the BR-2 precedence inversion — to ship undetected: every individual row was checked and the one combination that mattered, env opt-in *with* `enabled: false`, was covered by nothing. Precedence only exists between arms, so a per-arm enumeration cannot catch it. (BR-4)
