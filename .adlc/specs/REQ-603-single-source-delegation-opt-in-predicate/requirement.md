...
| gate calls per skill run | **1–2** (`proceed` 1, `wrapup` 1, `spec` 2, `analyze` 2) |
| median delegated step (telemetry, n=181) | **104,000 ms** (max 2,133,000 ms) |
...
- [ ] BR-5: `no-binary` (return 2) is decided in shell, before any probe, because it is the one question the probe cannot answer. This is not an exception to BR-1: binary resolution is not an authorizing arm — it can only withhold delegation, never grant it.
...
- [ ] `delegate-gate.sh` **does** contain the `ADLC_DISABLE_DELEGATE` veto, positioned before any probe invocation. (BR-2)
...
- [ ] Removing the `ADLC_DISABLE_DELEGATE` arm from `delegation_enabled()` causes the **Python** suite to fail and the direct-CLI path to transmit, while the gate's veto still holds — the BUG-209 regression, with each layer's responsibility asserted in its own suite. (BR-2, BR-9)
...
- [ ] `adlc-read --print-enabled` with no arguments still prints `1`/nothing exactly as before, verified against a caller written before this REQ. (BR-3)
...
- [ ] Telemetry rows written after the change carry the same `reason` values as rows written before, for the same conditions. (BR-4)

## Out of Scope

- **Changing the reason vocabulary.** BR-4 freezes all six values. Adding, renaming, or splitting a reason — including splitting `api-error`, which BUG-208 noted is overloaded — is a separate REQ.
...
- **The measured costs are representative.** 21 ms per probe and a 104-second median step were measured on this machine (macOS, Python 3.9, warm filesystem; 50 iterations; telemetry n=181). A cold cache or slower disk will differ. The conclusion is robust to a large error — the argument needs the probe to be ≪ the step, and it is by ~5000×.
- **Gate-call frequency stays at 1–2 per skill run.** Counted from `adlc_delegate_gate_check` call sites in `proceed`, `wrapup`, `spec`, and `analyze`. A future skill calling the gate in a loop would change the arithmetic and should re-open the caching question this REQ closed.
