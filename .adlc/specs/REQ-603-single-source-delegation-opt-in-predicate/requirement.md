...
- [ ] BR-3: the probe reports **both** verdict and reason, because `ADLC_DELEGATE_GATE_REASON` is a documented contract with downstream consumers — `partials/delegate-gate.md:59` tabulates the values and `agents/delegate-pre-pass.md:34-35,122` requires `gateReason` **verbatim** in its structured output. Note what is *not* a consumer: `tools/delegate/check-delegation.sh` aggregates the telemetry **`mode`** column (`:76-79`), not `reason`, and the telemetry `reason` field is a skill-level outcome vocabulary distinct from the gate's — over all 212 telemetry rows it carries `no-flag` (27) and `api-error` (24), which are not gate reasons, and carries `disabled-via-env` **zero** times. The two must not be conflated. A verdict-only probe would force the gate to re-derive the reason from the config it was just told not to read, reintroducing the duplication in a second form. The existing `--print-enabled` contract (prints `1`/nothing) keeps its `1`/`0` output shape (**not** byte-frozen — see BR-4 D5); the reason-bearing form is additive.
- [ ] BR-4: the gate's six reason strings and the 0/1/2 return-code shape are preserved, with **five named divergences** — enumerated here because "one deliberate change" was asserted in an earlier draft and was false. Every divergence fails *closed*; none makes the gate grant delegation it previously withheld.

  | # | input | pre-REQ | now | rc | status |
  |---|---|---|---|---|---|
  | D1 | `enabled: false`, no legacy key | `not-opted-in` | `disabled-via-config` | unchanged (1) | ADR-4, ratified 2026-09-01 |
  | D2 | key VALUE in `api_key_env` | `not-opted-in` | `disabled-via-config` | unchanged (1) | ratified 2026-09-01 |
  | D3 | opt-in satisfied, key var unset | `ok` | `disabled-via-config` | **0 → 1** | ratified 2026-09-01 |
  | D4 | config exists but unreadable | `ok` | `disabled-via-config` | **0 → 1** | ratified 2026-09-01 |
  | D5 | `--print-enabled`, config exists but unreadable, legacy key | prints `1` | prints `0` | output, not rc | ratified 2026-09-01 — inherited D4 via `resolve_provider().enabled`; earlier text claimed this flag was frozen in three places and was wrong |

  D1 corrects a label that depended on an unrelated variable: the pre-REQ helper never read `enabled`, returning `disabled-via-config` only when a config file existed *and* a legacy key happened to be set.

  D2 and D3 are both LESSON-392 — the probe must share the REAL call's resolution, not a cheaper subset. D2 was already caught by wrapping `resolve_provider`; D3 required also calling `resolve_key`, without which a machine with `enabled: true` and no key set reported `ok` while the real call died on it. D3 changes a return code, which is why it is named rather than folded into D2.

  D4 closes a fail-OPEN: an unreadable or unparsable config returned an empty dict, indistinguishable from an absent one, and fell through to legacy-key continuity. An operator who had written `enabled: false` on a machine with a stale key exported delegated anyway — BUG-205's reported outcome reached through a read failure instead of a precedence bug. A config that exists but cannot be read is now a refusal, not a default.

  A fifth behaviour is **new rather than divergent** and is named here so the table is not read as exhaustive of change: a probe that exceeds the 10s bound (where `timeout(1)` exists) is a non-zero exit and therefore `not-opted-in`. Pre-REQ there was no probe on most paths and `--print-enabled` was documented as never erroring, so no prior mapping existed to diverge from. It is fail-closed, but a wedged probe is a machine in an *unknown* state being recorded as an opt-in state, which is worth knowing when reading telemetry.

  Anything not in this table is byte-identical, asserted by the AC-19 matrix — now the full 24-row cross-product of veto × env-opt-in × config × legacy-key, with expected values derived from the documented ranking rather than transcribed from the implementation. The previous hand-listed 13 rows omitted 9 of the 12 veto combinations, which is how a legacy-key-over-veto inversion stayed invisible.
...
- [ ] BR-14 (**known limitation, discharged by REQ-609**): the config reader that this REQ makes the sole authority is a hand-written flat reader, and it **fails open on ordinary YAML shapes** — a comment on the `delegate:` header, a BOM, a tab-indented header, a nested mapping containing `enabled`, a second `delegate:` block, a directory at the config path. Each yields `{}`, indistinguishable from no config, and legacy-key continuity then grants against a written `enabled: false`. Every one of these is **pre-existing on `main`**: this REQ does not introduce them and does not fix them. An attempt to fix them in this REQ (commit `bc7d584`, discarded) added six further fail-opens under a tests-first discipline, because a reader that *skips* what it does not understand cannot be patched into one that *refuses*; see `adversary-report.md` passes 3 and 4. A real parser behind a strict schema, with a differential oracle, is REQ-609. **Beside it, the shell resolver**: `command -v` consults shell state, so a shell function (zsh), an alias, or a hash-table entry named `adlc-read` can satisfy binary resolution — also pre-existing on `main`, also REQ-609 (its BR-11). Until REQ-609 lands, the controls that hold are `enabled: false` in a config the minimal reader *can* parse, and the kill switch. **Discharged 2026-09-02 by REQ-609 (PR #149), both halves:** the parser by its ADR-1 — one loader parsing with `yaml.safe_load` behind a strict closed schema, with a differential oracle over a seeded and a generated corpus — and the resolver by its ADR-3, which walks `$PATH` itself over absolute entries only and never consults a lookup builtin, closing the class (function, alias, **and** hash table) rather than the bare-name mechanism.
...
- [ ] `adlc-read --print-enabled` keeps its `1`/`0` output shape and its behaviour on every row except BR-4 D5, asserted by `tests/test_pre_req_gate_parity.py::test_d5_print_enabled_is_not_frozen_and_the_spec_says_so` against the **frozen pre-REQ CLI** (`partials/tests/fixtures/pre-req-603/`), not a restatement. (BR-3)
...
- [ ] A vendored `delegate-gate.sh` predating this REQ **differs from canonical** — the property `/template-drift`'s partials-posture reports as `stale` — asserted by diffing the **frozen pre-REQ gate fixture** against the current file and requiring a non-empty diff; the current file against itself is empty (benign path). (BR-13)
- [ ] Both gates — the **frozen pre-REQ gate** and the current one — run over the full 24-row exported cross-product with the same Python behind them and differ on exactly the BR-4 rows; the Python-layer divergences D3/D4/D5 are asserted with the frozen pre-REQ CLI behind the frozen gate; and the malformed-config classes the matrix could not previously observe are rows too, so a class that does NOT diverge is recorded as such. `tests/test_pre_req_gate_parity.py`. (BR-4)
- [ ] `agents/delegate-pre-pass.md` continues to receive `gateReason` verbatim, with no value outside the frozen six. (BR-3, BR-4)
- [ ] **Exactly the five BR-4 divergences occur, and no others**, established by running the pre-REQ gate rather than a restatement of the ranking. The previous substitute compared the cascade against a restatement by the same author — it could prove the code matched its own description, never that it matched the pre-REQ gate. (BR-4)

## Out of Scope

- **The config parser, and shell-state-free binary resolution.** Both are REQ-609. This REQ names their present fail-opens as a known limitation in BR-14 rather than claiming them fixed. **Both landed 2026-09-02 in REQ-609 (PR #149)** — the parser via its ADR-1, the resolver via its ADR-3 — discharging BR-14.
