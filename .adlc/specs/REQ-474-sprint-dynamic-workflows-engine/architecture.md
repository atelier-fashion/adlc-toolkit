...
**CREATE**
- `workflows/adlc-sprint.workflow.js` — the deterministic orchestration script (meta + Preflight + per-REQ pipeline of Phases 0–8).
- `workflows/schemas.js` — the JSON-Schema literals (`REPOS`, `VERDICT`, `TASKS`, `FINDINGS`, `CANDIDATES`, `PRS`, `TERMINAL`) imported by the script.
- `agents/kimi-pre-pass.md` — the new per-repo pre-pass leaf agent (target; gated on OQ-1).
- `workflows/tests/` — minimal test harness + unit tests for the deterministic helpers.
...
**ADR-2 — Workflow scripts live in `workflows/`, reached via the skills symlink; `/init` vendors them.**
