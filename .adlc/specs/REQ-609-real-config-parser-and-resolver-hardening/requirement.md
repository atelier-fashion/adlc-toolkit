...
- [ ] With `ADLC_READ_BIN` empty, every skill call-site fence exits non-zero before any transmission, and the `delegate-pre-pass` agent's two fences refuse into its degraded object without transmitting — that agent's contract forbids a non-zero exit as a signal, so its refusal is the same stderr line plus `invoked:false`; and `grep -rn 'ADLC_READ_BIN:-adlc-read'` over the skills, agents, and partials matches nothing outside `partials/tests/fixtures/` and `.adlc/specs/`. (BR-12)
...
- **Two interpreters today, one after this REQ.** Verified 2026-09-01 and carried into BR-8: the delegate CLIs run in `~/.claude/delegate-venv`; `adlc`, the `adlc doctor` config probes, and `partials/forge.sh`'s reader run under `$PATH`'s `python3` unless the venv is preferred by the rule in ADR-2 (venv present **and** carrying PyYAML — a venv created before this REQ has `openai` and no PyYAML, so the rule must check the package, not the interpreter). PyYAML on `$PATH`'s `python3` is a user-site install on the reference machine, not an Apple-shipped one. `tools/adlc`'s own pytest suite keeps running under whichever interpreter invokes it, with PyYAML available because the venv pins it and the tests are run from there.
...
- **The probe's cost is measured, not assumed (AC-12).** Medians of 20 runs each, timed with `time.perf_counter()` around `subprocess.run`, on the reference machine (Apple arm64, macOS 26.6.2, CPython 3.9.6 in `~/.claude/delegate-venv`), against that machine's real `~/.claude/adlc/config.yml`:
  - `adlc-read --print-gate`: **before** (`main`, `e70a1f1`) median **21.2 ms** (min 20.6) → **after** (this branch) median **25.5 ms** (min 25.0). Delta **+4.3 ms** — the lazy `import yaml` plus the strict parse.
  - `adlc-read --help`, which never reaches the loader (BR-1): **before** median **21.6 ms** (min 21.3) → **after** median **19.5 ms** (min 19.2). Unchanged; the negative sign is run-to-run noise, not a speed-up.
  - Command: `~/.claude/delegate-venv/bin/python3 tools/delegate/adlc-read --print-gate` (and `--help`), 20 iterations each, medians and minima in milliseconds. The venv interpreter is invoked **directly** rather than through `~/bin/adlc-read`, which `exec`s the primary checkout — still on `main` — and would have measured the *before* code twice.
  - Against REQ-603's **104-second** median delegated step, +4.3 ms is **0.004%** of one delegated step: noise. That is what the pre-measurement assumption ("roughly thirty milliseconds", unmeasured) guessed and could not show; the guess was also for the import alone, while the measured delta covers the import and the parse together.
