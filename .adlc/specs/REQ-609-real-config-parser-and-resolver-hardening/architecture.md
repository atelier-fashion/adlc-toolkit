...
**Decision.** `load_machine_config(path=None) -> ConfigOutcome` lives in `tools/delegate/_machine_config.py`. `_common.py` imports it directly; `tools/adlc/forge_config.py` loads it by explicit file path (`importlib.util.spec_from_file_location`, cached, never touching `sys.path` — amended at Phase 5: a `HOME`-derived `sys.path[0]` insertion shadowed every later lazy import in the process, `yaml` included), first from `<file>/../../delegate/` via `realpath`, then from the installed skills copy under `~/.claude/skills/tools/delegate/` so a project that vendors `forge_config.py` alone (which `partials/forge.sh` supports) still resolves its config. ASSUME-001 (wrapper `exec` preserves script-relative resolution) is what the first candidate relies on.
...
**Outcome shape.** `ConfigOutcome(kind, document, reason, path)`. `kind ∈ {absent, parsed, malformed}`. `document` is the top-level mapping — `{}` for a null document (empty file, comments only) per REQ BR-3. `reason` is a short class string for `malformed` (`not-regular-file`, `dangling-symlink`, `unreadable`, `undecodable`, `over-cap`, `yaml-error`, `duplicate-key`, `alias-or-merge`, `not-a-mapping`, `dependency-missing`, and `schema`, emitted one layer up by the delegate adapter; `yaml-error` also covers the plain built-in exceptions PyYAML's constructors raise — a broad final catch, because the constructor set is not enumerable and "never raises" must hold) plus a human line that may carry a line number, never file content. `parse_delegate_config` maps `malformed` to `{_MALFORMED: True, _MALFORMED_REASON: <reason>, _MALFORMED_PATH: <path>}` so `require_delegation_enabled` can name both (BR-13) while every existing `cfg.get(_MALFORMED) is True` check keeps working.

**Duplicate keys, aliases, merge keys.** A `yaml.SafeLoader` subclass overrides `construct_mapping` to construct each key and raise with the second occurrence's mark on a repeat (the mark gives BR-13 its line number; whole-document by design, REQ BR-2). Amended at Phase 5: it also overrides `compose_node` to refuse any alias and `flatten_mapping` to refuse a merge key, reason class `alias-or-merge` — a `<<: *base` or `delegate: *x` could otherwise supply `enabled` without the word appearing under `delegate:`, and refusing aliases also removes the alias-expansion resource risk the first version accepted. Merges are never flattened.
...
**Decision.** One interpreter rule, applied in three places: the root `install.sh`'s `adlc` shim, `tools/adlc/checks.py`'s config probes, and `partials/forge.sh`'s reader invocation (the path `/proceed` uses for PR operations) all prefer `~/.claude/delegate-venv/bin/python3` when it is a regular executable file **and** that venv carries a `yaml` package under its `site-packages`, and fall back to `python3` from `$PATH` otherwise. The package check matters: a venv created before this REQ has `openai` and no PyYAML, and a rule that tested only the interpreter would move `adlc` from the interpreter that can parse the config to the one that cannot (Phase 5 finding; `install.sh` runs `ensure_adlc_shim` before any venv refresh). `forge.sh` no longer discards the reader's stderr, so the `dependency-missing` line reaches the operator. `adlc doctor` gains a `pyyaml` check that reports whether PyYAML imports in the interpreter the rule selects, says when the venv is being bypassed for lack of PyYAML, SKIPs when there is no venv and no config file to parse, and offers the copy-pasteable fix (`install.sh --with-delegation`; when a venv exists without PyYAML, `<venv>/bin/pip install -r <repo>/tools/delegate/requirements.txt` — never `pip install --user`, which a venv refuses and which would bypass the pin).
...
**Amended at Phase 5 (2026-09-02).** The reference machine's `$PATH` `python3` carries PyYAML from a user-site `pip install --user`, not from Apple; a fresh Mac has it in the venv (after `--with-delegation`) and nowhere else. The spec's Assumption was corrected. Also refused since Phase 5: YAML aliases and merge keys document-wide (a `<<: *base` could supply `enabled` without the word appearing under `delegate:`, and alias expansion was the one accepted resource risk).
