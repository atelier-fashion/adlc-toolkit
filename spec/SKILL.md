...
7. **Body-read of top-15 docs** — gated delegation, hard fallback. Decide via:
   ...
   **Delegated body-read** (gate passes — `ask-kimi` is on PATH and `ADLC_DISABLE_KIMI` is not `1`):
   ...
   5. **Claude post-validation (BR-3, load-bearing — LESSON-007):** the summary is a *proposal*. Before relying on any cited id or path, sanitize the citation tokens with strict regexes — reject (do not just `ls`) anything else to prevent path traversal via Kimi-injected strings:
   ...
   6. The orchestrator works off the validated summary plus the frontmatter list already produced in sub-steps 4–6. **Do NOT read the full body of any top-15 doc in this branch** — Kimi's summary replaces that read.
