...
- **GUI-launched Claude Code can't see `MOONSHOT_API_KEY`** — `install.sh` now
  installs a LaunchAgent (`com.adlc-toolkit.kimi-setenv`) that runs at every login
  and re-populates the launchctl session env from your `~/.zshrc` / `~/.bash_profile`,
  so the GUI-inheritance problem self-heals after every reboot. If you change the key
  value in your rc and want it picked up immediately, run `launchctl setenv
  MOONSHOT_API_KEY "$MOONSHOT_API_KEY"` once to update the current session (or log
  out + back in for the LaunchAgent to do it for you).
...
- **Inspect the LaunchAgent** — `launchctl list | grep adlc-toolkit` confirms it's
  loaded; `cat /tmp/kimi-launchctl-setenv.log` shows what it did at the last login
  (path checked, key length — never the key value itself).
